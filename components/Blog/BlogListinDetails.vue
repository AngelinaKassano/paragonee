        ...
        <div v-for="(blog, index) in blogData.slice(0, 4)" :key="index">
            <div class="flex sm:items-center flex-col sm:flex-row">
                ...
                <div class="xl:ml-9 lg:ml-6 md:ml-9 ml-3 flex flex-col flex-1">
                    ...
                    <n-link :to="`/blog/${blog.slug}`" class="uppercase font-exo relative pl-12 text-sm text-white transition-all hover:text-primary before:empty-content before:absolute before:h-0.5 before:w-7.5 before:bg-primary before:left-0 before:top-1/2 before:transform before:-translate-y-1/2">Подробнее</n-link>
                </div>
            </div>
        </div>
    </div>
